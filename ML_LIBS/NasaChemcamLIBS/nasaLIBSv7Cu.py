        ...
        for name in ['avg1','avg2','avg3','avg4','avg5']:
            ...
            for culine in test_cu_line:
                for feline in test_fe_line:
                    try:
                        xtemp.append(max(getROI(culine, key)[name]) / max(meanTrainingData[key][name]))
...
if __name__=='__main__':
    ...
    REPEAT_TIMES = 100
    if not os.path.exists("E:\\LIBS_experiment\\" + element + 'v7_version0529max'):
        os.mkdir("E:\\LIBS_experiment\\" + element + 'v7_version0529max')
    os.chdir("E:\\LIBS_experiment\\" + element + 'v7_version0529max')
