...
#寻找对应元素特征峰作为特征,并根据该光谱中的最大值normalize
"""
:param 寻找元素
:return 对应元素的特征序列
...
def selectFeature(element):
    ...
    for samplename,concentrate in concentrate_set_200AVG.items():
        ...
        MAX = data_set_200AVG[samplename].Intensity.max()
        for feature_list in element_dict[element]:
            ...
            templist.append(feature_value/MAX)

        featurelist.append(templist)
    for samplename,concentrate in concentrate_set_1000AVG.items():
        templist = []
        #第一项为特征波长，第二项为importance
        MAX = data_set_1000AVG[samplename].Intensity.max()
        for feature_list in element_dict[element]:
...
def elementTest(element, x, y, flag, featureCompressRate, times=10):
    ...
    for i in range(0, 10):
        ...
        file.write('Part 2 Experiment with Random forest regression---------------------------------')
        ...
        if len(x[0]) < 10:
            ...
            for i in range(0, len(x[0])):
                ...
                file.write("importance is " + str(importance[indices[i]]))
                if flag:
                    print("对应的特征峰和importance为 " + str(extract_element_dict[element][indices[i]]))
                    file.write("对应的特征峰和importance为 " + str(extract_element_dict[element][indices[i]]))
        else:
            # 打印前十的importance
            for i in range(0, 10):
                print("importance is " + str(importance[indices[i]]))
        ...
        file.write('Part 3 LASSO experiment ---------------------------------')
