...
def newMain():
    ...
    for key, val in meanTrainingData.items():
        for name in ['avg1', 'avg2', 'avg3', 'avg4', 'avg5']:
            ...
            for line in test_al_line:
                try:
                    xtemp.append(max(getROI1(line,key)[name]) / max(meanTrainingData[key][name]))
            ...
            x.append(xtemp)
            y.append(con.loc[con.Name == key]['Al2O3'].values[0])
...
if __name__ == '__main__':
    ...
    element = 'Al'
    REPEAT_TIMES = 100
    if not os.path.exists("E:\\LIBS_experiment\\" + element + 'v7_mixed'):
        os.mkdir("E:\\LIBS_experiment\\" + element + 'v7_mixed')
    os.chdir("E:\\LIBS_experiment\\" + element + 'v7_mixed')
