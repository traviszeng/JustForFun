...
#标准化
def normalize(data):
    norm = (data - data.mean()) / (data.max() - data.min())*100
    #print(norm)
    return norm
...
def test(element):
    ...
    for info in element_info:
        ...
        for key, item in meanTrainingData.items():
            ...
            if key not in trainingCase:
                trainingCase[key] = []
                trainingCase[key].append(getROI(aim,key))
            else:
                trainingCase[key].append(getROI(aim, key))
    ...
    for key, item in trainingCase.items():
        for name in trainingCase[key][0].columns :
            if name!='wave':
                xx =[]
                for index in range(0,len(trainingCase[key])):
                    if len(trainingCase[key][index][name])==0:
                        xx.append(0)
                    else:
                        xx.append(max(trainingCase[key][index][name]))
                x.append(xx)
